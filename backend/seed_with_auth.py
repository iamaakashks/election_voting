...
def seed_data():
    ...
    for branch in branches:
        for section in sections:
            ...
            for i in range(1, students_per_combination + 1):
                ...
                # Email format: usn@nie.edu.in
                email = f"{usn.lower()}@nie.edu.in"
                
                # Demo password: last 4 characters of USN
                demo_password = usn[-4:]
            ...
            db.commit()
            db.close()
            print(f"✓ Completed {branch}-{section}: {students_per_combination} students")
    
    # Create admin account with known credentials
    db = SessionLocal()
    
    # Delete existing admin if any
    db.query(Student).filter(Student.is_admin == True).delete()
    
    admin = Student(
        usn="ADMIN001",
        email="admin@nie.edu.in",
    ...
    db.commit()
    db.close()
    print("\n✓ Admin account created (USN: ADMIN001, Email: admin@nie.edu.in, Password: admin123)")
    ...
    for branch in branches:
        for section in sections:
            ...
            if sample:
                demo_pwd = sample.usn[-4:]
                print(f"  {branch}-{section}: {sample.name}")
                print(f"    Email: {sample.email}")
                print(f"    USN: {sample.usn}")
                print(f"    Password: {demo_pwd} (last 4 of USN)")
                print()
    
    db.close()
    print("\n✅ Seeding complete!")
    print("\n🔐 LOGIN CREDENTIALS:")
    print("  Student: Use college email (e.g., 4ni22csa001@nie.edu.in)")
    print("  Password: Last 4 digits of USN (e.g., 001)")
    print("  Admin: admin@nie.edu.in / admin123")
